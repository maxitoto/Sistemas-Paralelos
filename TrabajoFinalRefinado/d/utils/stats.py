...
def init(hardware, software, time_baseline, save_baseline, save_estimate_baseline=False):
    ...
    if save_baseline or save_estimate_baseline:
        _config["time_baseline"] = 1.0
        print("📊 [Stats] Inicializado. (Calculando NUEVO Baseline...)")
    else:
        _config["time_baseline"] = float(time_baseline) if time_baseline else 1.0
        print(f"📊 [Stats] Inicializado. Usando Baseline en caché: {_config['time_baseline']}s")
